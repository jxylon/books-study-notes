...
### $\xi8.5.5$ AdaBoost与LR的关系

第一次提到AdaBoost和LR 的关系是本书参考文献， 给出了Boosting和LR损失函数之间的关系， 但是里面用到的损失小二乘。

本书的参考文献，从Bregman散度的角度解释AdaBoost和LR的关系。

文献中有说明，LR的特征对应了AdaBoost中的弱分类器，或者是基分类器，分类器对应了hypotheses。
